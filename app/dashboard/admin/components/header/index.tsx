...
import { AssignAdminsideTrue } from "@/lib/store/slices/Allslices";
...
const Header = () => {
  ...
  return (
    <div className="w-full sticky z-20 top-0 h-16 bg-white shadow-sm flex items-center p-5 justify-between">
      <span onClick={() => dispatch(AssignAdminsideTrue())}>
      <HiBars3 className="text-2xl text-P_textColour flex cursor-pointer lg:hidden"  />
      </span>
      ...
      <div className="flex justify-end">
        <div className="flex items-center md:space-x-5 space-x-2">
          <div className="sm:flex hidden w-max">
              ...
              <div className="shadow-card flex h-[46px] w-[82px] items-center justify-center rounded-md bg-white">
                <span
                  className={`flex h-9 w-9 items-center justify-center rounded ${
                    !isChecked ? "bg-blue-600 text-white" : "text-body-color"
                  }`}
                >
                  <svg
                    width="16"
          ...
          <div className="rounded-full h-8 w-8 flex justify-center items-center bg-[rgb(239,244,251)] cursor-pointer">
            <AiOutlineBell className="text-gray-500 text-xl" />
          </div>
          <div className="rounded-full h-8 w-8 bg-[rgb(239,244,251)] flex justify-center items-center cursor-pointer">
            <BiMessageRoundedDots className="text-gray-500 text-xl" />
          </div>
          <div className="w-max flex space-x-3 h-max items-center">
            <div className="sm:flex hidden w-max flex-col items-end">
              <h1 className="text-sm text-HeadingColours whitespace-nowrap">Noble Bridge</h1>
              <p className="text-xs text-P_textColour">Role</p>
            </div>
            <div className="w-1/2 flex items-center space-x-3 cursor-pointer">
              <div className="h-max w-12 rounded-full">
                <Image src={userimg} alt="User.png" />
              </div>
              <BsChevronDown />
            </div>
