...
const Orders = () => {
  ...
  useEffect(() => {
    ...
    else if(category==="Panding"){
      setData(
        products.filter((e:any, i:any) => {
          return e.status === 'Panding'
        })
      )
    }
  },[category])
    ...
    <>
      ...
      :<>
          <div className=" sm:px-10 px-3">
            <h1 className="text-2xl text-ubuntu-bold mb-5 text-HeadingColours">Orders</h1>
  ...
  <tbody className="bg-white divide-y divide-gray-200">
    {data &&
      data.map((product, i) => {
        const { image, name, price, status, stock, sales, market, type, vender } = product;
        return (
          <tr key={i}>
            ...
            <td className="px-6 py-4 whitespace-nowrap text-xs text-gray-500">
              <div className={`p-1 px-2 rounded-lg w-max text-ubuntu-regular ${status === "Delivered" ? "bg-[#CDFEE1] text-[#083D25]" : "bg-[#E0F0FF] text-[#0E57A2]"}`}>
                {status}
              </div>
            </td>
